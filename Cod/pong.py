...
placar_fonte = pygame.font.Font("./font/Pixeltype.ttf", 50)
placar_jogador = 0
placar_adversario = 0
...
placar_adversario_rect = placar_adversario_texto.get_rect(center=(largura_tela - 30, 50))
...
while True:
        ...
        if event.type == pygame.QUIT:
            ...
            if event.key == pygame.K_f: # tecla F para mostrar o FPS
                ...
                jogador_pos, adversario_pos, bola_pos, bola_vel = utils.reset(
                    centro_tela, largura_tela, jogador_vel
                )
    ...
    if jogo_rodando:
        ...
        if jogador.colliderect(pygame.Rect(bola_pos.x - bola_r, bola_pos.y - bola_r, bola_r * 2, bola_r * 2)):
            bola_vel.x = abs(bola_vel.x)
        if adversario.colliderect(pygame.Rect(bola_pos.x - bola_r, bola_pos.y - bola_r, bola_r * 2, bola_r * 2)):
            bola_vel.x = -abs(bola_vel.x)
        ...
        if bola_pos.x - bola_r <= 0:
            ...
            placar_adversario_rect = placar_adversario_texto.get_rect(center=(largura_tela - 30, 50))
            bola_pos = pygame.Vector2(centro_tela)
            bola_vel = pygame.Vector2(bola_velocidade * random.choice([-1, 1]), bola_velocidade * random.choice([-1, 1]))
        ...
        if modo_jogo == "computador":
            ...
            if keys[pygame.K_DOWN]:
                adversario_pos.y += jogador_vel
            adversario_pos.y = max(0, min(adversario_pos.y, altura_tela - jogador_rect_altura))
